...
Write a neutral, factual summary for each article — no opinion, no editorializing, just what happened and why it matters competitively. Format is uniform across every section (see `references/format.md`): one active-voice sentence per article, with the whole thing hyperlinked — no separate headline/summary split, and no section gets a longer paragraph treatment than any other.
